...
def sliding_window_search(binary_warped, histogram, debug_display=False):
    """Sliding window search for lane detection with performance optimizations.
    
    Args:
        binary_warped: Binary thresholded warped image
        histogram: Histogram of lower half of image
        debug_display: If True, creates visualization (optional overhead)
    ...
    for window in range(nwindows):
        ...
        # Only draw rectangles if visualization is enabled
    ...
    # Arrays are already pre-allocated, no need to concatenate
    ...
    try:
        if len(leftx) > 0 and len(rightx) > 0:
            ...
            if debug_display and out_img is not None:
                ...
                out_img[nonzeroy[left_inds_int], nonzerox[left_inds_int]] = [255, 0, 0]
                out_img[nonzeroy[right_inds_int], nonzerox[right_inds_int]] = [0, 0, 255]
        else:
